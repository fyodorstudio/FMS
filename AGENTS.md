...
2. **Zero Outcome Faking**: Never use modulo arithmetic (`% 2`, `% 3`), binary toggles, or hardcoded strings to generate trade outcomes or chart badges.
...
> *"I cannot perform this operation because the required historical data is not present on disk. Please provide the dataset or authorize its ingestion."*
...
- The Python bridge, MT5 integration, economic-calendar ingestion, persistent storage, and desktop packaging require separate explicit authorization.
...
- `economic-calendar/calendar-dock` owns the calendar table shown in the bottom dock.
...
- `trader-notebook/notebook-dock` owns the manual trade planner, pip calculator, and durable journal notes panel.
- `trader-notebook/chart-levels` owns the projected Entry, Take Profit, and Stop Loss horizontal price lines on the chart.
...
- The sibling `bridge/` directory is a read-only local data adapter. Keep persistence, order functions, and frontend presentation out of it.
...
- Never block the UI thread with market-data access.
