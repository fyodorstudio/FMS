...
- [ ] **Single Drawing Deletion (`Delete` / `Backspace` & UI action)**
...
- [ ] **Instrument-Aware Pip / Point Calculation in Position Tool**
...
- [ ] **Incremental Bar Updates (`series.update`)**
  - In `useMt5MarketData`, utilize `series.update(lastBar)` for regular live candle polls (2-second interval).
  - Reserve full `series.setData(bars)` calls strictly for initial load, timeframe/symbol switching, or 60-second reconciliation.
  - Reduces chart redraw overhead and ensures smooth animation during high-frequency price ticks.

- [ ] **Visible Logical Range Preservation During History Paging**
...
- [ ] **Economic Event Markers on Chart Time Axis**
...
- [ ] **Active Terminal Process Selection Display**
